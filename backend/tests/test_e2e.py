...
MERGE_RESPONSE = json.dumps({
    "findings": [{"description": "Combined: malicious extension confirmed on multiple hosts", "severity": "high", "iocs": [], "mitre_technique": "T1195.002"}],
    "new_iocs": [],
    "reasoning": "All three sub-agents confirm supply chain compromise via trojanized extension",
})
...
async def test_full_investigation_pipeline():
    ...
    mock_merge_llm = MagicMock()
    mock_merge_llm.ainvoke = AsyncMock(return_value=AIMessage(content=MERGE_RESPONSE))
    ...
    with patch("app.agent.nodes.detect.get_llm", return_value=mock_detect_llm), \
         patch("app.agent.nodes.sub_agents.get_llm", side_effect=lambda: _make_sub_agent_llm()), \
         patch("app.agent.nodes.sub_agents.run_splunk_query", AsyncMock(return_value="[]")), \
         patch("app.agent.nodes.sub_agents._get_mcp_tools", AsyncMock(return_value={})), \
         patch("app.agent.nodes.investigate.get_llm", return_value=mock_merge_llm), \
